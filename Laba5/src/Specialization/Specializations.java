 ...
 * @author Sofia
 */
public class Specializations extends javax.swing.JDialog {

    Connection connection;
    SpecializationModel model;
    int columnWidth=50;
    static int currentIdSpecialization;
    
    public Specializations(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;
        currentIdSpecialization = SpecializationModel.selectSpecializations(c).size()+1;
        model = new SpecializationModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        ...
        try {
            NewSpecialization ns = new NewSpecialization((Frame) this.getParent(), true, connection,currentIdSpecialization);
            ns.setVisible(true);
            model.updateData();
            currentIdSpecialization++;
        } catch (SQLException ex) {
            Logger.getLogger(Specializations.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        try { if (jTable1.getSelectedRow() >= 0) {
            NewSpecialization ns = new NewSpecialization((Frame) this.getParent(), true,
