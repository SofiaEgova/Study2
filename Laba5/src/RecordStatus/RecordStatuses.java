 ...
 * @author Sofia
 */
public class RecordStatuses extends javax.swing.JDialog {

    Connection connection;
    RecordStatusModel model;
    int columnWidth=50;
    static int currentIdService;
    
    public RecordStatuses(java.awt.Frame parent, boolean modal, Connection c) throws SQLException {
        super(parent, modal);
        initComponents();
        setLocationRelativeTo(null);
        connection = c;
        currentIdService = RecordStatusModel.selectRecordStatuses(c).size()+1;
        model = new RecordStatusModel(connection);
    ...
    private void jButtonADDActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonADDActionPerformed
        ...
        try {
            NewRecordStatus ns = new NewRecordStatus((Frame) this.getParent(), true, connection,currentIdService);
            ns.setVisible(true);
            model.updateData();
            currentIdService++;
        } catch (SQLException ex) {
            Logger.getLogger(RecordStatuses.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private void jButtonALTERActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonALTERActionPerformed
        try { if (jTable1.getSelectedRow() >= 0) {
            NewRecordStatus np = new NewRecordStatus((Frame) this.getParent(), true,
